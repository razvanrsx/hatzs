...
let chatQuestions = document.getElementById('chat-questions');
...
const QUICK_QUESTIONS = [
  ...
  'How do I assign a device to a user?',
  'What triggers overconsumption alerts?',
...
async function loadMonitoring(targetDeviceId) {
  ...
  try {
    ...
    state.monitoring = (allowedDeviceIds
      ...
      const recordTime = new Date(record.hourStart).getTime();
      return Number.isNaN(recordTime) ? false : recordTime >= sevenDaysAgo;
    });
...
function ensureChatQuestionsContainer() {
  if (chatQuestions || !supportSection) {
    return;
  }
  const wrapper = document.createElement('div');
  wrapper.className = 'chat-questions';
  wrapper.innerHTML = `
    <p class="muted">Quick questions:</p>
    <div id="chat-questions" class="chat-questions-list"></div>
  `;
  const history = supportSection.querySelector('#chat-history');
  if (history) {
    history.insertAdjacentElement('beforebegin', wrapper);
  } else {
    supportSection.appendChild(wrapper);
  }
  chatQuestions = wrapper.querySelector('#chat-questions');
}
